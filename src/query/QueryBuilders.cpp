...
Query::Ptr FakeQueryBuilder::tryExtractQuery(TokenizedQueryString *query) {
  ...
  if (count != 4)
    std::cerr << std::endl;
  return this->get_nextBuilder()->tryExtractQuery(query);
}
...
void ComplexQueryBuilder::parseToken(TokenizedQueryString *query) {
  ...
  if (*it != "WHERE")
    ...
    throw IllFormedQuery(
        std::string(R"(Expecting "WHERE", found "?".)"_f % it->c_str()));
  while (++it != end) {
    if (*it != "(")
...
Query::Ptr ComplexQueryBuilder::tryExtractQuery(TokenizedQueryString *query) {
  ...
  std::string const operation =
      query->token
          .front(); // the first string refers to the operation of the query
  if (operation == "INSERT")
    return std::make_unique<InsertQuery>(
        this->get_target_table(), this->operandToken, this->conditionToken);
  if (operation == "UPDATE")
  ...
  if (operation == "SUM")
    // return std::make_unique<NopQuery>(); // Not implemented
    return std::make_unique<SumQuery>(this->get_target_table(),
                                      this->operandToken, this->conditionToken);
  if (operation == "MIN")
    // return std::make_unique<NopQuery>(); // Not implemented
    // 10.25 test
    return std::make_unique<MinQuery>(this->get_target_table(),
                                      this->operandToken, this->conditionToken);
  if (operation == "MAX")
    // return std::make_unique<NopQuery>(); // Not implemented
    return std::make_unique<MaxQuery>(this->get_target_table(),
                                      this->operandToken, this->conditionToken);
  if (operation == "ADD")
    // return std::make_unique<NopQuery>(); // Not implemented
    return std::make_unique<AddQuery>(this->get_target_table(),
                                      this->operandToken, this->conditionToken);
  if (operation == "SUB")
    // return std::make_unique<NopQuery>(); // Not implemented
    return std::make_unique<SubQuery>(this->get_target_table(),
                                      this->operandToken, this->conditionToken);
  if (operation == "SWAP")
    return std::make_unique<SwapQuery>(this->get_target_table(),
                                       this->operandToken,
  ...
  for (const auto &oprand : this->operandToken)
    std::cerr << oprand << " ";
  std::cerr << std::endl;
  std::cerr << "Target Table = " << this->get_target_table() << std::endl;
...
void ComplexQueryBuilder::clear() {
  ...
  this->get_target_table() = "";
  this->operandToken.clear();
  this->get_nextBuilder()->clear();
}
