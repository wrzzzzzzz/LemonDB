...
// the function that finds local minimums of 20 rows, i.e.
// local_minimums: the vector of minimum values over the 20 lines i.e., (112
// 2014) values: a vector of the rows of local minimums, i.e., 2 3 4 5 (do we
// create a table type or create an array of ints?)
void MinQuery::findmin(Table *table,
                       std::vector<Table::FieldIndex> field_indexes, int left,
                       int right = -1) {
  std::vector<Table::ValueType> local_minimums;
  auto itbegin = table->origin_begin() + left;
  if (itbegin.deleted())
    ++itbegin;
  if (right == -1) { // till the end of table
    for (unsigned int j = 0; j < field_indexes.size(); j++) {
      int temp = table->ValueTypeMax;
      int flag = 0;
      ...
      if (flag == 1) {
        local_minimums.push_back(temp);
      }
    }
    std::lock_guard<std::mutex> const lock(mtx);
    if (local_minimums.size() != 0) {
      this->values.push_back(local_minimums);
    }
...
// e.g. MIN ( totalCredit class ) FROM Student WHERE ( class > 2014 );
//  should operate based on given columns, say, fieldnames
// this refers to a query
QueryResult::Ptr MinQuery::execute() {
  ...
  try {
    ...
    // e.g. MIN ( totalCredit class ) FROM Student WHERE ( class > 2013 );
    ...
    if (result.second) { // meet the WHERE sentence(?)
      ...
      if (NUMOFTHREADS == 1) {
        ...
        for (int i = 0; i < NUMOFTHREADS; i++) {
          if (i == NUMOFTHREADS - 1) {
            threads.push_back(
                std::thread(std::bind(&MinQuery::findmin, this, &table,
                                      field_indexes, i * size, -1)));
          } else {
            assert(i * size <= static_cast<int>(table.size()));
            threads.push_back(std::thread(std::bind(&MinQuery::findmin, this,
                                                    &table, field_indexes,
                                                    i * size, (i + 1) * size)));
          }
        }
        ...
        // find the minima of each column
...
std::string MinQuery::toString() {
  return "QUERY = MIN " + this->get_target_table() + "\"";
}
