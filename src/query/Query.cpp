  ...
  for (auto &cond : condition) {
    if (cond.field == "KEY") {
      ...
      try {
        ...
        throw IllFormedQueryCondition(std::string(
            R"("?" is not a valid condition operator.)"_f % cond.op.c_str()));
      }
      switch (op) {
      case '>':
...
//   // Correct placement of casting const table to non-const
//   Table &nonConstTable = const_cast<Table &>(table);

//   auto condResult = initCondition(nonConstTable); // use nonConstTable here
//   if (!condResult.second) {
//     function(false, nullptr);
//     return true;
//   }
//   if (!condResult.first.empty()) {
//     auto object = nonConstTable[condResult.first]; // and here
